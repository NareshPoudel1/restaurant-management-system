...
def index(request):
    ...
    naresh = {'allProds': allProds}
    return render(request, 'shop/index.html',naresh)
...
def recommended_products(request):
    return render(request, 'shop/recommended_products.html')
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    naresh = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        naresh = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', naresh)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            naresh_dict['CHECKSUMHASH'] = Checksum.generate_checksum(naresh_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'darshan_dict': naresh_dict})
